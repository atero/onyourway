(function(angular) {
    ...
    var paths = {
        ...
        register: '/users/sign_up.json',
        resetpassword:'/users/password.json',
        updatepassword:'/users/password.json'
    };
    ...
    this.$get = ['$q', '$http', '$rootScope', function($q, $http, $rootScope) {
        ...
        var service = {
            ...
            /**
             * A resetpassword function to send a reset password email
             * with the server. Keep in mind, credentials are sent
             * in plaintext; use a SSL connection OUT secure them.
             * By default, `resetpassword` will POST to '/users/password.json'.
             *
             * The path and HTTP method used to login are configurable
             * using
             *
             ...
             */
             resetpassword:function(creds){
                ...
                return $http(httpConfig('resetpassword', creds))
                    .then(service.parse)
                    .then(broadcast('resetpassword'));

             },
             /**
             * A updatepassword function to register the new passwords on the server
             * with the server. Keep in mind, credentials are sent
             * in plaintext; use a SSL connection OUT secure them.
             ...
             *                  rejected by the server.
             */
             updatepassword:function(creds){
                creds = creds || {};
                return $http(httpConfig('updatepassword', creds))
                    // .then(service.parse)
                    .then(broadcast('updatepassword'));
